...
>
> 原始碼以 **MIT** 授權開源，作為一份參考實作與作品集——歡迎閱讀、學習，或自行部署。**本專案已不再維護，亦不提供任何技術支援。** `caretaiwan.app` 網域將不再續約，到期後失效。
...
**程式碼：[MIT](./LICENSE)。** 版權所有 © 2026 GUOPEICHI（YuriCrystal）。

自由使用、研究、修改、發布——商用或非商用都可以——保留版權聲明就好，沒有其他限制。

**貢獻：** 歡迎 PR，貢獻同樣以 MIT 授權納入。
